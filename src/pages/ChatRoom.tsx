...
const ChatRoom = ({ match }: RouteComponentProps<MatchUserPostParams>) => {
  ...
  return (
    <IonPage>
      <IonHeader mode="ios">
        <IonToolbar mode='ios' onClick={() => { console.log("HI"); if ("uid" in contactInfo && contactInfo.uid) dynamicNavigate(router, '/about/' + schoolName + "/" + contactInfo.uid, 'forward') }}>
          {contactInfo &&
              ...
              <>
                {messages?.map((msg: any, index: number) => (
                  <FadeIn key={msg.uid + '_' + index.toString()} transitionDuration={500}>
                    <ChatMessage msg={msg} school={schoolName} toggled={false} photo={contactPhoto} />
                  </FadeIn>
                ))}
                {kbHeight != 0 ?
                  <>
